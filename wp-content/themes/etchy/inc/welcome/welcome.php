...
if ( ! class_exists( 'EtchyWelcomePage' ) ) {
	class EtchyWelcomePage {
		private static $instance;

		/**
		 * get the instance of EtchyWelcomePage
		 *
		 * @return self
		 */
		...
		function init_activation_hook() {
			if ( ! is_network_admin() ) {
				set_transient( '_etchy_welcome_page_redirect', 1, 30 );
			}
		}
		...
		function welcome_page_redirect() {
			...
			if ( ! get_transient( '_etchy_welcome_page_redirect' ) ) {
				return;
			}

			// delete the redirect transient
			delete_transient( '_etchy_welcome_page_redirect' );
			...
			wp_safe_redirect( add_query_arg( array( 'page' => 'etchy_welcome_page' ), esc_url( admin_url( 'themes.php' ) ) ) );
			exit;
		}
		...
		function create_welcome_page() {
			...
			remove_submenu_page( 'themes.php', 'etchy_welcome_page' );
		}
		...
		function welcome_page_content() {
			...
			$params['theme_screenshot']  = file_exists( ETCHY_ROOT_DIR . '/screenshot.png' ) ? ETCHY_ROOT . '/screenshot.png' : ETCHY_ROOT . '/screenshot.jpg';

			etchy_template_part( 'welcome', 'templates/welcome', '', $params );
		}
		...
		function enqueue_styles( $hook ) {
			if ( $hook === 'appearance_page_etchy_welcome_page' ) {
				wp_enqueue_style( 'etchy-welcome-page-style', ETCHY_INC_ROOT . '/welcome/assets/admin/css/welcome.min.css' );
			}
		}
	}
}

EtchyWelcomePage::get_instance();