...
function mql_render_presupuesto_publico()
{
    ...
    ob_start();
    ?>
    <div id="mql-presupuesto-publico" class="mql-presupuesto-publico">
        <form id="mql-presupuesto-publico-form">
            ...
            <p>
                ...
                <input type="text" name="titulo" value="Presupuesto web" required>
            </p>

            <p>
                <label>Encuadernación</label>
                <select name="e_encu" required>
            ...
            <fieldset>
                ...
                <p>
                    <label>Páginas interior</label>
                    <input type="number" name="0e_paginas" min="1" step="1" required>
                </p>

                <p>
                    <label>Papel interior</label>
                    <select name="0e_tipo_papel" required>
                ...
                <p>
                    <label>Tintas interior</label>
                    <select name="0e_tintas" required>
            ...
            <fieldset>
                ...
                <p>
                    <label>Papel cubierta</label>
                    <select name="2e_tipo_papel" required>
                ...
                <p>
                    <label>Tintas cubierta</label>
                    <select name="2e_tintas" required>
                ...
                <p>
                    <label>Plastificado cubierta</label>
                    <select name="2e_plast">
        ...
        <div id="mql-presupuesto-publico-result" style="margin-top:20px;"></div>
    </div>

    <script>
        jQuery(function ($) {
            $('#mql-presupuesto-publico-form').on('submit', function (e) {
                e.preventDefault();

                const $form = $(this);
                const $result = $('#mql-presupuesto-publico-result');
                ...
                $.ajax({
                    ...
                    method: 'POST',
                    dataType: 'json',
                    data: $form.serialize() + '&action=mql_presupuesto_publico',
                    success: function (response) {
                        if (!response.success) {
                            $result.html('<div class="mql-error">' + response.data.message + '</div>');
                            return;
                        }

                        const data = response.data;
                        let html = '<div class="mql-ok">';
...
function mql_presupuesto_publico_ajax()
{
    if (
        empty($_POST['nonce']) ||
        !wp_verify_nonce(sanitize_text_field($_POST['nonce']), 'mql_presupuesto_publico_nonce')
    ) {
        wp_send_json_error(['message' => 'Petición no válida.']);
    }

    $cantidad = isset($_POST['quantity']) ? absint($_POST['quantity']) : 0;
    $paginas_interior = isset($_POST['0e_paginas']) ? absint($_POST['0e_paginas']) : 0;

    if ($cantidad <= 0 || $paginas_interior <= 0) {
    ...
    try {
        ...
        if (empty($priceRequest) || !is_array($priceRequest)) {
            wp_send_json_error(['message' => 'Optimus no ha devuelto precio.']);
        }

        $first = reset($priceRequest);

        if (!empty($first['error_message'])) {
            wp_send_json_error([
        ...
        wp_send_json_success($first);

    } catch (Throwable $e) {
        wp_send_json_error([
            'message' => 'Error al calcular el presupuesto: ' . $e->getMessage(),
        ]);
    }
}