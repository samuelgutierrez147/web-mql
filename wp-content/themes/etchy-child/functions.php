...
/* =========================================================
 * MQL - Selector público de presupuesto con producto YITH WAPO
 * Shortcode: [mql_selector_presupuesto]
 * Cliente Optimus fijo para modo público: PRUEBAS
 * ========================================================= */

/**
 * 1) Configura aquí los productos permitidos.
 * Sustituye los IDs por los IDs reales de WooCommerce.
 */
function mql_productos_presupuesto_publico()
{
    return [
        101 => 'Encuadernación Rústica Pur',
        102 => 'Encuadernación Rústica Cosida',
        103 => 'Encuadernación Tapa Dura Pur',
        104 => 'Encuadernación Grapado',
        105 => 'Encuadernación Espiral',
        106 => 'Encuadernación Wire-o',
        107 => 'Encuadernación Tapa Dura Lomo Cuadrado',
        108 => 'Encuadernación Tapa Dura Espiral',
        109 => 'Encuadernación Tapa Dura Wire-o',
        110 => 'Personalizado',
    ];
}

/**
 * 2) Cifrado / descifrado del token de URL.
 */
...
function mql_encrypt_public_quote_payload(array $payload)
{
    $method = 'AES-256-CBC';
...
function mql_decrypt_public_quote_payload($token)
{
...
function mql_get_public_quote_payload_from_request()
{
    if (empty($_REQUEST['mqlq'])) {
        return false;
    }

    return mql_decrypt_public_quote_payload($_REQUEST['mqlq']);
}

function mql_is_valid_public_quote_request_for_product($product_id = null)
{
    $payload = mql_get_public_quote_payload_from_request();

    if (!$payload) {
        return false;
    }

    if (($payload['cliente'] ?? '') !== 'PRUEBAS') {
        return false;
    }

    if ($product_id !== null && (int) ($payload['product_id'] ?? 0) !== (int) $product_id) {
        return false;
    }

    $allowed = mql_productos_presupuesto_publico();

    return isset($allowed[(int) $payload['product_id']]);
}

/**
 * 3) Shortcode del selector previo.
 */
add_shortcode('mql_selector_presupuesto', 'mql_render_selector_presupuesto');

function mql_render_selector_presupuesto()
{
    if (!function_exists('wc_get_product')) {
        return '<p>WooCommerce no está disponible.</p>';
    }

    $productos = mql_productos_presupuesto_publico();

    ob_start();
    ?>
    <div class="mql-selector-presupuesto">
        <form id="mql-selector-presupuesto-form" method="post">
            <h3>Calcula tu presupuesto</h3>

            <p>
                <label for="mql_product_id">Selecciona el tipo de producto</label>
                <select id="mql_product_id" name="product_id" required>
                    <option value="">Selecciona una opción</option>
                    <?php foreach ($productos as $product_id => $label): ?>
                        <?php
    ...
    <script>
        jQuery(function ($) {
            $('#mql-selector-presupuesto-form').on('submit', function (e) {
                e.preventDefault();

                const productId = $('#mql_product_id').val();
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    data: {
                        action: 'mql_get_public_product_quote_url',
                        product_id: productId,
                        nonce: '<?php echo esc_js(wp_create_nonce('mql_public_quote_selector')); ?>'
                    },
                    success: function (response) {
                        if (!response.success || !response.data.url) {
                            alert(response.data && response.data.message ? response.data.message : 'No se ha podido abrir el producto.');
                            return;
                        }
    ...
    <style>
        ...
        .mql-selector-presupuesto label {
            display: block;
            margin-bottom: 8px;
            font-weight: 600;
        }

        .mql-selector-presupuesto select {
            width: 100%;
            padding: 12px;
            border-radius: 8px;
        }

        .mql-selector-presupuesto button {
            margin-top: 16px;
            padding: 12px 18px;
            border: 0;
            border-radius: 10px;
...
/**
 * 4) AJAX que genera la URL cifrada.
 */
add_action('wp_ajax_mql_get_public_product_quote_url', 'mql_get_public_product_quote_url');
add_action('wp_ajax_nopriv_mql_get_public_product_quote_url', 'mql_get_public_product_quote_url');

function mql_get_public_product_quote_url()
{
    if (
        empty($_POST['nonce']) ||
        !wp_verify_nonce(sanitize_text_field($_POST['nonce']), 'mql_public_quote_selector')
    ) {
        wp_send_json_error(['message' => 'Petición no válida.']);
    }

    $product_id = absint($_POST['product_id'] ?? 0);
    $productos = mql_productos_presupuesto_publico();

    if (!$product_id || !isset($productos[$product_id])) {
        wp_send_json_error(['message' => 'Producto no permitido.']);
    }

    $product = wc_get_product($product_id);

    if (!$product || $product->get_status() !== 'publish') {
        wp_send_json_error(['message' => 'El producto no existe o no está publicado.']);
    }
    ...
    $token = mql_encrypt_public_quote_payload($payload);

    if (!$token) {
        wp_send_json_error(['message' => 'No se ha podido generar el acceso cifrado.']);
    }

    $url = add_query_arg('mqlq', rawurlencode($token), get_permalink($product_id));
