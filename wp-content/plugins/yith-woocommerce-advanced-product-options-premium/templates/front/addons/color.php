...
>
    ...
    <div class="label <?php echo wp_kses_post(!empty($addon_image_position) ? 'position-' . $addon_image_position : ''); ?>">
        ...
        <div class="color-container">
			<span class="color"
                  style="<?php echo in_array($color_type, array('color', 'single', 'double')) ? 'background:' . wp_kses_post($background) : ''; ?>">
				<?php
                if ('image' === $color_type) {
                    echo '<img src="' . esc_attr($addon->get_option('color_image', $x)) . '" alt="Color swatch image">';
                }
                ?>
			</span>
