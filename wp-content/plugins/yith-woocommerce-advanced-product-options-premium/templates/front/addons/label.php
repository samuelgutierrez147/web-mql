...
$addon_label = str_replace('*', '', $addon_label);
...
//[MQL] - MEJORAR
$type = 'type="checkbox"';
$elementosRadio = ['formato', 'orientacion'];  // Claves predefinidas para tipo radio

// Comprobar si custom_id coincide con algún valor de elementosRadio o termina con '_tipo_impresion', '_tintas' o '_tipo_papel'
if (in_array($custom_id, $elementosRadio) || preg_match('/(_tipo_impresion|_tintas|_tipo_papel)$/', $custom_id)) {
    $type = 'type="radio"';
    $required = true;
}
...
<div id="yith-wapo-option-<?php echo esc_attr($addon->id); ?>-<?php echo esc_attr($x); ?>"
    ...
    <!-- INPUT -->
    <?php $value = $addon->options["label_in_cart_opt"][$x]; ?>
    <input <?= $type ?>
        ...
        <?php if ($required) echo 'data-required="true"'; ?>
        <?php
        if ($default_price > 0) {
            ?>
    ...
    <div class="label_container <?php echo esc_attr($images_position); ?> <?php echo esc_attr($label_position_style); ?> <?php echo 'content-align-' . wp_kses_post($label_content_align_style) ?>">
        ...
        <div class="label <?php echo wp_kses_post(!empty($addon_image_position) ? 'position-' . $addon_image_position : ''); ?>"
             for="yith-wapo-<?php echo esc_attr($addon->id . '-' . $x); ?>">
