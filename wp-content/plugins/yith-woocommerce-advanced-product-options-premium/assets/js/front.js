            ...
            checkNumbersTotalValues = function () {
                ...
                numberAddons.each(function (index) {
                    ...
                    let numberAddon = $(this),
                        ...
                        numberStep = numberAddon.data('numbers-step'),
                        totalCount = 0,
                        errorCheck = false,
                        errorMessage = '',
                    ...
                    if ('undefined' !== typeof numberStep && totalCount % numberStep !== 0) {
                        errorCheck = true;
                        errorMessage = "Las páginas no son múltiplo de " + numberStep;
                    }
                    ;
...
function yith_wapo_check_required_fields(action) {
    ...
    function toggleErrorState(element, addonTitle, toggleAddonTitle, showError) {
        if (showError) {
    ...
    jQuery('form.cart .yith-wapo-addon:not(.hidden):visible input, form.cart .yith-wapo-addon:not(.hidden):visible select').each(function () {
        let element = jQuery(this);
        let groupName = element.attr('name');
        let parent = element.closest('.yith-wapo-option');
        let addon = element.closest('.yith-wapo-addon');
        let toggleAddon = element.closest('div.yith-wapo-addon.wapo-toggle');
        let toggleAddonTitle = toggleAddon.find('h3.wapo-addon-title.toggle-closed');
        let addonTitle = addon.find('.wapo-addon-title');

        if (['file', 'wapo-product-qty'].includes(element.attr('type'))) {
            return;
        }

        if (element.attr('type') === 'radio') {
            let optionsSelected = jQuery(`input[type="radio"][name="${groupName}"]`).closest('.yith-wapo-option').hasClass('selected');

            toggleErrorState(element, addonTitle, toggleAddonTitle, !optionsSelected);
            if (!optionsSelected) isRequired = true;
        } else if (
            ...
            if (action === 'highlight') {
                toggleErrorState(element, addonTitle, toggleAddonTitle, true);
            }
            hideButton = true;
            isRequired = true;
        } else {
            toggleErrorState(element, addonTitle, toggleAddonTitle, false);
        }
    });

    if (action === 'hide') {
        hideButton ? jQuery(buttonClasses).hide() : jQuery(buttonClasses).fadeIn();
    }
...
showRequiredMessage = function (element) {
    ...
    let optionFind = option.closest('.yith-wapo-addon');
    let addonTitle = optionFind.find('.addon-header');

    // Verificar si el mensaje de error ya existe
    if (optionFind.find('.required-error').length === 0) {
        // Si no existe, agrega el mensaje de error debajo del título
        addonTitle.after(
            '<div class="required-error">' +
            '<small class="required-message" style="color: red;">' + yith_wapo.messages.requiredMessage + '</small>' +
            '</div>'
        );

        // Añade la clase required-color para resaltar el campo con error
...
function yith_wapo_check_min_max(addon, submit = false) {
    ...
    let optionFind = addon.closest('.yith-wapo-addon');
    addonTitle.removeClass('wapo-error');
    optionFind.find('.required-error').remove();
