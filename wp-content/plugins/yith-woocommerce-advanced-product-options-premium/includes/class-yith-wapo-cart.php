	...
	class YITH_WAPO_Cart {
				...
				foreach ( $cart_item['yith_wapo_options'] as $index => $option ) {
					foreach ( $option as $key => $value ) {
						if ( $key && '' !== $value ) {
							...
							$addon_visible   = $this->get_addon_visible_value_on_cart( $addon_id, $option_id, $key, $value, $cart_item, $grouped_in_cart );
							...
							// Ahora asignamos los valores correctos a 'display' y 'visible'
							$display_value = empty( $option_price ) ? $addon_value : '<div>' . $this->get_addon_display_on_cart( $addon_id, $option_id, $addon_value, $sign, $option_price, $addon_price, $addon_sale_price ) . '</div>';
							$visible_value = empty( $option_price ) ? $addon_visible : '<div>' . $this->get_addon_display_on_cart( $addon_id, $option_id, $addon_visible, $sign, $option_price, $addon_price, $addon_sale_price ) . '</div>';
		
							if ( 'yes' === get_option( 'yith_wapo_show_options_in_cart', 'yes' ) ) {
								if ( ! $is_empty_select ) {
									if ( ! isset( $cart_data_array[ $addon_data_name ] ) ) {
										$cart_data_array[ $addon_data_name ] = array(
											'display' => '',
											'visible' => ''
										);
									}
		
									$cart_data_array[ $addon_data_name ]['display'] .= '<div>' . $display_value . '</div>';
									$cart_data_array[ $addon_data_name ]['visible'] .= '<div>' . $visible_value . '</div>';
								}
							}
				...
				if ( ! $grouped_in_cart ) {
					foreach ( $cart_data_array as $key => $value ) {
						...
						$cart_data[] = array(
							...
							'visible' => $value['visible'], // Asigna el mismo valor a visible
						);
					}
				}
			}
		...
		public function get_addon_display_on_cart( $addon_id, $option_id, $value, $sign, $price, $regular_price, $sale_price ) {
            ...
            if ( ! $hide_products_prices ) {
                $display = $value . ( '' !== $price && floatval( 0 ) !== floatval( $price ) ? ' (' . strip_tags( $sign . wc_price( $price ) ) . ')' : '' );
            } else {
                $display = $value;
            }
		...
		public function get_addon_value_on_cart( $addon_id, $option_id, $key, $original_value, $cart_item, $grouped_in_cart = false ) {
			...
			if ( 'product' === $addon_type ) {
				...
				if ( ! $grouped_in_cart && ! $is_empty_title ) {
					...
					$value = $label . ( ! empty( $original_value ) ? $original_value : __( 'No value provided', 'text-domain' ) );
				} else {
					$value = ! empty( $original_value ) ? $original_value : __( 'No value provided', 'text-domain' );
				}
			...
			// Generar el valor para data-value
			$data_value = ! empty( $label_in_cart_opt ) ? esc_attr($label_in_cart_opt) : esc_attr($value);

			return apply_filters( 'yith_wapo_get_addon_value_on_cart', $data_value, $addon_id, $option_id, $key, $original_value, $cart_item );
		}


		public function get_addon_visible_value_on_cart( $addon_id, $option_id, $key, $original_value, $cart_item, $grouped_in_cart = false ) {
			...
			// Generar el valor visible en el HTML
			$visible_value = esc_html($value);

			return apply_filters( 'yith_wapo_get_addon_visible_value_on_cart', $visible_value, $addon_id, $option_id, $key, $original_value, $cart_item );
		}
