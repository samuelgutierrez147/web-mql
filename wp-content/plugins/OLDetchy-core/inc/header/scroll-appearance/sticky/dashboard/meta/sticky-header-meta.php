...
if ( ! function_exists( 'etchy_core_add_sticky_header_meta_options' ) ) {
	function etchy_core_add_sticky_header_meta_options( $section, $custom_sidebars ) {
		...
		if ( $section ) {
			...
			$section->add_field_element(
				array(
					...
					'title'       => esc_html__( 'Choose Custom Sticky Header Widget Area', 'etchy-core' ),
					'description' => esc_html__( 'Choose custom widget area to display in sticky header widget area', 'etchy-core' ),
	...
	add_action( 'etchy_core_action_after_header_scroll_appearance_meta_options_map', 'etchy_core_add_sticky_header_meta_options', 10, 2 );
}

if ( ! function_exists( 'etchy_core_add_sticky_header_logo_meta_options' ) ) {
	function etchy_core_add_sticky_header_logo_meta_options( $logo_tab, $header_logo_section ) {
	...
	add_action( 'etchy_core_action_after_page_logo_meta_map', 'etchy_core_add_sticky_header_logo_meta_options', 10, 2 );
}