...
class EtchyCoreTopArea {
	...
	public function __construct() {
		...
		add_action( 'etchy_action_before_page_header', array( $this, 'load_template' ) );
		...
		add_filter( 'etchy_filter_add_inline_style', array( $this, 'set_inline_top_area_styles' ) );
	}
	...
	function is_top_area_enabled() {
		$top_area_enabled = etchy_core_get_post_value_through_levels( 'qodef_top_area_header' ) === 'yes';
	...
	public function set_top_area_height() {
		if ( $this->is_top_area_enabled ) {
			$height         = etchy_core_get_post_value_through_levels( 'qodef_top_area_header_height' );
			$top_bar_height = ! empty( $height ) ? $height : 32;
	...
	public function is_top_area_transparent() {
		$background_color = etchy_core_get_post_value_through_levels( 'qodef_top_area_header_background_color' );
	...
	public function load_template() {
		...
		etchy_core_template_part( 'header/top-area/', 'templates/top-area', '', $parameters );
	}
	...
	public function set_inline_top_area_styles ( $style ) {
        ...
        $height       = etchy_core_get_post_value_through_levels ( 'qodef_top_area_header_height' );
        $side_padding = etchy_core_get_post_value_through_levels ( 'qodef_top_area_header_side_padding' );
