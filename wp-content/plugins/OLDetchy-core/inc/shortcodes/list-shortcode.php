...
abstract class EtchyCoreListShortcode extends QodeFrameworkShortcode {
	private $post_type;
	...
	public function register_list_scripts() {
		...
		$list_scripts = apply_filters( 'etchy_core_filter_register_list_shortcode_scripts', isset( $scripts ) ? $scripts : array() );
	...
	public function map_list_options( $params = array() ) {
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'behaviour', $exclude_behavior ) ) ) {
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'behavior',
				'title'         => esc_html__( 'List Appearance', 'etchy-core' ),
				'options'       => etchy_core_get_select_type_options_pool( 'list_behavior', false, $exclude_behavior ),
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'masonry', $exclude_behavior ) ) ) {
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Image Proportions', 'etchy-core' ),
				'options'       => array(
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'images_proportion', $exclude_option ) ) ) {
			$this->set_option( array(
				...
				'default_value' => 'full',
				'title'         => esc_html__( 'Image Proportions', 'etchy-core' ),
				'options'       => etchy_core_get_select_type_options_pool( 'list_image_dimension', false ),
				'group'         => $group,
				'dependency'    => array(
					'show' => array(
						'behavior' => array(
			...
			$this->set_option( array(
				...
				'title'       => esc_html__( 'Custom Image Width', 'etchy-core' ),
				'description' => esc_html__( 'Enter image width in px', 'etchy-core' ),
				'group'       => $group,
				'dependency'  => array(
					'show' => array(
			...
			$this->set_option( array(
				...
				'title'       => esc_html__( 'Custom Image Height', 'etchy-core' ),
				'description' => esc_html__( 'Enter image height in px', 'etchy-core' ),
				'group'       => $group,
				'dependency'  => array(
					'show' => array(
						'images_proportion' => array(
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'columns', $exclude_option ) ) ) {
			$this->set_option( array(
				...
				'options'       => etchy_core_get_select_type_options_pool( 'columns_number', true, $exclude_columns, $include_columns ),
				'default_value' => '3',
				'group'         => $group,
				'dependency'    => array(
					'hide' => array(
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'justified-gallery', $exclude_behavior ) ) ) {
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Row Height', 'etchy-core' ),
				'dependency' => array(
					'show' => array(
						'behavior' => array(
							'values'        => 'justified-gallery',
			...
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Max Row Height', 'etchy-core' ),
				'dependency' => array(
					'show' => array(
						'behavior' => array(
							'values'        => 'justified-gallery',
			...
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Last Row Treshold', 'etchy-core' ),
				'dependency' => array(
					'show' => array(
						'behavior' => array(
							'values'        => 'justified-gallery',
	...
	public function map_slider_options( $params = array() ) {
		...
		$this->set_option( array(
			'field_type'  => 'text',
			'name'        => 'slider_speed',
			'title'       => esc_html__( 'Slide Duration', 'etchy-core' ),
			'description' => esc_html__( 'Default value is 5000 (ms)', 'etchy-core' ),
			'dependency'  => $dependency,
			'group'       => $group
		) );
		$this->set_option( array(
			...
			'title'       => esc_html__( 'Slide Animation Duration', 'etchy-core' ),
			'description' => esc_html__( 'Speed of slide animation in milliseconds. Default value is 800.', 'etchy-core' ),
			'dependency'  => $dependency,
			'group'       => $group
		) );
		$this->set_option( array(
		...
		$this->set_option( array(
			'field_type' => 'select',
			'name'       => 'slider_navigation_type',
			'title'      => esc_html__( 'Slider Navigation Type', 'etchy-core' ),
			'options'    => array(
				''        => esc_html__( 'Inside', 'etchy-core' ),
				'outside' => esc_html__( 'Outside', 'etchy-core' )
			),
			'dependency' => $dependency,
			'group'      => $group
		) );
		...
		$this->set_option( array(
			...
			'title'      => esc_html__( 'Slider Pagination Type', 'etchy-core' ),
			'options'    => array(
				''        => esc_html__( 'Inside', 'etchy-core' ),
				'outside' => esc_html__( 'Outside', 'etchy-core' )
			),
		...
		$this->set_option( array(
			'field_type' => 'select',
			'name'       => 'slider_layout',
			'title'      => esc_html__( 'Slider Layout', 'etchy-core' ),
			'options'    => array(
	...
	public function get_list_classes( $atts ) {
		...
		$holder_classes[] = ! empty( $atts['slider_navigation_type'] ) ? 'qodef-swiper-navigation--' . $atts['slider_navigation_type'] : '';
		$holder_classes[] = ! empty( $atts['slider_pagination_type'] ) ? 'qodef-swiper-pagination--' . $atts['slider_pagination_type'] : '';
	...
	public function get_list_item_image_dimension( $atts ) {
		...
		if ( ! empty( $atts['behavior'] ) && $atts['behavior'] == 'masonry' && ! empty( $atts['masonry_images_proportion'] ) && $atts['masonry_images_proportion'] == 'fixed' ) {
			...
			$image_dimension              = etchy_core_get_custom_image_size_meta( 'meta-box', $masonry_image_dimension_name, get_the_ID() );
		}
	...
	public function get_slider_data( $atts, $include = array() ) {
		...
		$data['spaceBetween']   = isset( $atts['space'] ) ? ( etchy_core_get_space_value( $atts['space'] ) * 2 ) : 0; // double half space for slider
		...
		if ( isset( $atts['slider_layout'] ) && $atts['slider_layout'] === 'predefined' ) {
			$data['slidesPerView']  = 2.75;
			$data['centeredSlides'] = true;
			$data['slideToClickedItem'] = true;
		}
	...
	public function map_query_options( $params = array() ) {
		$group                = isset( $params['group'] ) ? $params['group'] : esc_html__( 'Query', 'etchy-core' );
		...
		$this->set_option( array(
			'field_type'    => 'text',
			'name'          => 'posts_per_page',
			'title'         => esc_html__( 'Posts per Page', 'etchy-core' ),
			'default_value' => '-1',
			'group'         => $group
		) );
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'orderby',
			'title'         => esc_html__( 'Order By', 'etchy-core' ),
			'options'       => etchy_core_get_select_type_options_pool( 'order_by', true, $exclude_order_by, $include_order_by ),
			'default_value' => 'date',
			'group'         => $group
		) );
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'order',
			'title'         => esc_html__( 'Order', 'etchy-core' ),
			'options'       => etchy_core_get_select_type_options_pool( 'order' ),
			'default_value' => 'DESC',
			'group'         => $group
		) );
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'additional_params', $exclude_option ) ) ) {
			...
			do_action( 'etchy_core_action_map_query_options_before_additional', $group );
			
			$additional_params = apply_filters( 'etchy_core_filter_map_additional_query_params', array(
				''       => esc_html__( 'No', 'etchy-core' ),
				'id'     => esc_html__( 'Post IDs', 'etchy-core' ),
				'tax'    => esc_html__( 'Taxonomy Slug', 'etchy-core' ),
				'author' => esc_html__( 'Author Name', 'etchy-core' ),
			) );
			...
			$this->set_option( array(
				'field_type'  => 'text',
				'name'        => 'post_ids',
				'title'       => esc_html__( 'Posts IDs', 'etchy-core' ),
				'description' => esc_html__( 'Separate post IDs with commas', 'etchy-core' ),
				'group'       => $group,
				'dependency'  => array(
					'show' => array(
			...
			if ( ! empty( $taxonomies_formatted ) ) {
				...
				$this->set_option( array(
					'field_type' => 'text',
					'name'       => 'tax_slug',
					'title'      => esc_html__( 'Taxonomy Slug', 'etchy-core' ),
					'group'      => $group,
					'dependency' => array(
						'show' => array(
							'additional_params' => array(
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'author_slug',
				'title'      => esc_html__( 'Author Slug', 'etchy-core' ),
				'group'      => $group,
				'dependency' => array(
					'show' => array(
						'additional_params' => array(
			...
			do_action( 'etchy_core_action_map_query_options_after_additional', $group );
		}
	}
	...
	public function get_additional_query_args( $atts ) {
		...
		$args = apply_filters( 'etchy_core_filter_additional_query_args', $args, $atts );
	...
	public function map_layout_options( $params = array() ) {
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'text_transform', $exclude_option ) ) ) {
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'text_transform',
				'title'      => esc_html__( 'Title Text Transform', 'etchy-core' ),
	...
	public function map_additional_options( $params = array() ) {
		$group_name         = esc_html__( 'Additional', 'etchy-core' );
		...
		if ( ! $exclude_filter ) {
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'enable_filter',
				'title'         => esc_html__( 'Enable Filter', 'etchy-core' ),
				'description'   => esc_html__( 'Enabling this option will show categories filter above list', 'etchy-core' ),
				'options'       => etchy_core_get_select_type_options_pool( 'no_yes' ),
				'default_value' => '',
				'dependency'    => array(
					'show' => array(
						'behavior' => array(
							'values'        => array( 'masonry' ),
							'default_value' => 'columns'
						)
					)
				),
			...
			do_action( 'etchy_core_action_map_additional_options_after_filter', $group_name );
		}
		...
		if ( ! $exclude_pagination ) {
			...
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Load More Top Margin (px or %)', 'etchy-core' ),
				'default_value' => '',
				'group'         => $group_name,
				'dependency'    => array(
					'show' => array(
			...
			do_action( 'etchy_core_action_map_additional_options_after_pagination', $group_name );
		}
	}
	...
	public function load_assets() {
		do_action( 'etchy_core_action_list_shortcodes_load_assets', $this->get_atts() );
	}
