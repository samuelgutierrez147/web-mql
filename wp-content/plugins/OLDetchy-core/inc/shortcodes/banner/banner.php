...
if ( ! function_exists( 'etchy_core_add_banner_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function etchy_core_add_banner_shortcode( $shortcodes ) {
		$shortcodes[] = 'EtchyCoreBannerShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'etchy_core_filter_register_shortcodes', 'etchy_core_add_banner_shortcode' );
}

if ( class_exists( 'EtchyCoreShortcode' ) ) {
	class EtchyCoreBannerShortcode extends EtchyCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'etchy_core_filter_banner_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'etchy_core_filter_banner_extra_options', array() ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( ETCHY_CORE_SHORTCODES_URL_PATH . '/banner' );
			$this->set_base( 'etchy_core_banner' );
			$this->set_name( esc_html__( 'Banner', 'etchy-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds banner element', 'etchy-core' ) );
			...
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Additional Image', 'etchy-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'link_url',
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'subtitle_tag',
				'title'         => esc_html__( 'Subtitle Tag', 'etchy-core' ),
				'options'       => etchy_core_get_select_type_options_pool( 'title_tag' ),
				'default_value' => 'h5',
				'group'         => esc_html__( 'Content', 'etchy-core' )
			) );
		...
		public function render( $options, $content = null ) {
			...
			return etchy_core_get_template_part( 'shortcodes/banner', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
