...
if ( ! function_exists( 'etchy_core_add_image_with_text_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function etchy_core_add_image_with_text_shortcode( $shortcodes ) {
		$shortcodes[] = 'EtchyCoreImageWithTextShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'etchy_core_filter_register_shortcodes', 'etchy_core_add_image_with_text_shortcode' );
}

if ( class_exists( 'EtchyCoreShortcode' ) ) {
	class EtchyCoreImageWithTextShortcode extends EtchyCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'etchy_core_filter_image_with_text_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'etchy_core_filter_image_with_text_extra_options', array() ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( ETCHY_CORE_SHORTCODES_URL_PATH . '/image-with-text' );
			$this->set_base( 'etchy_core_image_with_text' );
			$this->set_name( esc_html__( 'Image With Text', 'etchy-core' ) );
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'etchy_core_image_with_text', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return etchy_core_get_template_part( 'shortcodes/image-with-text', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
