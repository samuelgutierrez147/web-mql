...
if ( ! function_exists( 'etchy_core_add_icon_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function etchy_core_add_icon_shortcode( $shortcodes ) {
		$shortcodes[] = 'EtchyCoreIconShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'etchy_core_filter_register_shortcodes', 'etchy_core_add_icon_shortcode', 8 );
}

if ( class_exists( 'EtchyCoreShortcode' ) ) {
	class EtchyCoreIconShortcode extends EtchyCoreShortcode {
		
		public function map_shortcode() {
			$this->set_shortcode_path( ETCHY_CORE_SHORTCODES_URL_PATH . '/icon' );
			$this->set_base( 'etchy_core_icon' );
			$this->set_name( esc_html__( 'Icon', 'etchy-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon element', 'etchy-core' ) );
			...
			$this->set_option( array(
				'field_type'	=> 'iconpack',
				'name'			=> 'main_icon',
				'title'			=> esc_html__( 'Icon', 'etchy-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'size',
				...
				'options'    => array(
					...
					'tiny'    => esc_html__( 'Tiny', 'etchy-core' ),
					'small'   => esc_html__( 'Small', 'etchy-core' ),
					'medium'  => esc_html__( 'Medium', 'etchy-core' ),
					'large'   => esc_html__( 'Large', 'etchy-core' ),
					'huge'    => esc_html__( 'Huge', 'etchy-core' ),
				)
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'custom_size',
				'title'      => esc_html__( 'Custom Size', 'etchy-core' ),
			) );
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'icon_layout',
				...
				'options'       => array(
					'normal' => esc_html__( 'Normal', 'etchy-core' ),
					'circle' => esc_html__( 'Circle', 'etchy-core' ),
					'square' => esc_html__( 'Square', 'etchy-core' )
				),
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'border_radius',
				'title'      => esc_html__( 'Border Radius', 'etchy-core' ),
				'dependency' => array(
					'show' => array(
						'icon_layout' => array(
							'values'        => 'square',
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'shape_size',
				'title'      => esc_html__( 'Shape Size', 'etchy-core' ),
				'dependency' => array(
					'show' => array(
						'icon_layout' => array(
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'border_width',
				'title'      => esc_html__( 'Border Width', 'etchy-core' ),
				'dependency' => array(
					'show' => array(
						'icon_layout' => array(
							'values'        => array( 'circle', 'square' ),
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'etchy_core_icon', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return etchy_core_get_template_part( 'shortcodes/icon', 'templates/icon', '', $atts );
		}
