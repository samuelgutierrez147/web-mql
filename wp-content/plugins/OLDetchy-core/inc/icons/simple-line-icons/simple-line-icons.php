...
if ( ! function_exists( 'etchy_core_add_simple_line_icons_to_collection' ) ) {
	function etchy_core_add_simple_line_icons_to_collection( $icons ) {
		$icons[] = 'EtchyCoreSimpleLineIconsPack';
		return $icons;
	}
	
	add_filter( 'qode_framework_filter_add_icon', 'etchy_core_add_simple_line_icons_to_collection' );
}

if ( class_exists( 'QodeFrameworkIconPack' ) ) {
	class EtchyCoreSimpleLineIconsPack extends QodeFrameworkIconPack {
		...
		public function get_style_url() {
			return ETCHY_CORE_INC_URL_PATH . '/icons/' . $this->get_base() . '/assets/css/' . $this->get_base() . '.min.css';
		}
