...
if ( ! function_exists( 'etchy_core_register_breadcrumbs_title_layout' ) ) {
	function etchy_core_register_breadcrumbs_title_layout( $layouts ) {
		$layouts['breadcrumbs'] = 'EtchyCoreBreadcrumbsTitle';
		
		return $layouts;
	}
	
	add_filter( 'etchy_core_filter_register_title_layouts', 'etchy_core_register_breadcrumbs_title_layout');
}

if ( ! function_exists( 'etchy_core_add_breadcrumbs_title_layout_option' ) ) {
	/**
	 ...
	 * @return array
	 */
	function etchy_core_add_breadcrumbs_title_layout_option( $layouts ) {
		$layouts['breadcrumbs'] = esc_html__( 'Breadcrumbs', 'etchy-core' );
		
		return $layouts;
	}
	
	add_filter( 'etchy_core_filter_title_layout_options', 'etchy_core_add_breadcrumbs_title_layout_option' );
}

if ( ! function_exists( 'etchy_core_breadcrumbs' ) ) {
	/**
	 * Function that renders breadcrumbs html
	 */
	function etchy_core_breadcrumbs() {
		$page_id = qode_framework_get_page_id();
		
		// Breadcrumbs label
		$labels = apply_filters( 'etchy_core_filter_breadcrumbs_label', array(
			'home'        => esc_html__( 'Home', 'etchy-core' ),
			'tag'         => esc_html__( 'Posts tagged "%s"', 'etchy-core' ),
			'author'      => esc_html__( 'Posted by %s', 'etchy-core' ),
			'search'      => esc_html__( 'Search results for "%s"', 'etchy-core' ),
			'404'         => esc_html__( '404 - Page not found', 'etchy-core' ),
			'query_paged' => esc_html__( '(Page %s)', 'etchy-core' )
		) );
		
		// Breadcrumbs variables
		$settings = apply_filters( 'etchy_core_filter_breadcrumbs_settings', array(
		...
		$breadcrumbs_html = '';
		if ( ! empty( $wrap ) ) {
			$breadcrumbs_html = $settings['wrap_before'] . $wrap . apply_filters( 'etchy_core_filter_breadcrumbs_content', $wrap_child, $settings ) . $settings['wrap_after'];
		}
		
		echo apply_filters( 'etchy_core_filter_breadcrumbs_template', $breadcrumbs_html );
	}
}