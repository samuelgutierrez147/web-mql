...
if ( ! function_exists( 'wc_display_item_meta' ) ) {
	...
	function wc_display_item_meta( $item, $args = array() ) {
		...
		$strings = array(); // Inicializar el array de strings

		foreach ( $item->get_all_formatted_meta_data() as $meta_id => $meta ) {
			// Filtrar solo los meta que necesitamos
			$display_key = wp_kses_post( $meta->display_key );

			if ( 
				$display_key === 'Personalizado' || 
				$display_key === 'Título del Libro' || 
				strpos($display_key, 'Cantidades: Cantidad') !== false 
			) {
				$value     = $args['autop'] ? wp_kses_post( $meta->display_value ) : wp_kses_post( make_clickable( trim( $meta->display_value ) ) );
				$strings[] = $args['label_before'] . $display_key . $args['label_after'] . $value;
			}
		}
...
function wc_get_formatted_cart_item_data( $cart_item, $flat = false ) {
    ...
    // Verifica si el producto es una variación y que $cart_item['variation'] es un array
    if ( $cart_item['data']->is_type( 'variation' ) && is_array( $cart_item['variation'] ) ) {
        foreach ( $cart_item['variation'] as $name => $value ) {
            ...
            $item_data[] = array(
                'key'     => $label,
                'value'   => $value,
                'visible' => $visible_value, // Usa el valor calculado para 'visible'
            );
        }
    }
    ...
    if ( is_array( $item_data ) ) {
        foreach ( $item_data as $key => $data ) {
            ...
            // Preservar todas las claves existentes, incluyendo 'visible'
            $item_data[ $key ] = array_merge( $data, array(
                'key'     => ! empty( $data['key'] ) ? $data['key'] : $data['name'],
                'display' => ! empty( $data['display'] ) ? $data['display'] : $data['value'],
                'visible' => ! empty( $data['visible'] ) ? $data['visible'] : 'prueba',
            ));
        }
