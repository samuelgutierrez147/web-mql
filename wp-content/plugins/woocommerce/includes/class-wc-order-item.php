...
// Calcula el valor visible
$visible_value = $this->calculate_visible_value($meta->key, $display_value);
...
$formatted_meta[$meta->id] = (object) array(
	...
	private function calculate_visible_value($key, $display_value) {
		$product = $this->get_product();
		
		error_log("Calculando valor visible para la clave: " . $key);
		error_log("Valor de display: " . $display_value);
		
		if ($product && $product->is_type('variation')) {
			error_log("El producto es una variación");
			$attributes = $product->get_attributes();
			error_log("Atributos del producto: " . print_r($attributes, true));
			
			foreach ($attributes as $attribute_name => $attribute) {
				$taxonomy = wc_attribute_taxonomy_name(str_replace('attribute_pa_', '', urldecode($attribute_name)));
				error_log("Comprobando atributo: " . $attribute_name . " (Taxonomía: " . $taxonomy . ")");
				
				if (wc_attribute_label($taxonomy) === $key) {
					error_log("Coincidencia encontrada para la clave: " . $key);
					if (taxonomy_exists($taxonomy)) {
						$term = get_term_by('slug', $attribute, $taxonomy);
						if (!is_wp_error($term) && $term && $term->name) {
							error_log("Valor visible encontrado en taxonomía: " . $term->name);
							return $term->name;
						}
					} else {
						$filtered_value = apply_filters('woocommerce_variation_option_name', $attribute, null, $taxonomy, $product);
						error_log("Valor visible después del filtro: " . $filtered_value);
						return $filtered_value;
					}
				}
			}
		} else {
			error_log("El producto no es una variación o no se pudo obtener");
		}
		
		$meta_visible = $this->get_meta('visible');
		error_log("Valor de meta 'visible': " . ($meta_visible ? $meta_visible : 'No encontrado'));
		
		$visible_value = !empty($meta_visible) ? $meta_visible : 'prueba';
		error_log("Valor visible final: " . $visible_value);
